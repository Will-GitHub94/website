...
const reportAndExit = (message) => {
	if (errors.length) {
		...
		console.log(chalk.green("The next time your application starts, Mongoose will rebuild the index"
			+ ` "${indexToRemove}."`));
		process.exit(0);
	}
};
...
mongoose.connect((db) => {
	...
	console.log();
	console.log(chalk.yellow(`Removing index "${indexToRemove}"`
								+ " from the User collection."));
	console.log();

	// Remove the index
	userCollection.dropIndex(indexToRemove, (err, result) => {
